...
class HeatmapWrapper(gym.Wrapper):
    def __init__(self, env, model, extractor, layer, mode=1):
        gym.Wrapper.__init__(self, env)
        self.env = env
        self._mode = mode
        self._max_episode_steps = env._max_episode_steps
        ...
        # Prepare CAM
        if model == 'resnet18':
            self._cam_model = resnet18(weights=ResNet18_Weights.DEFAULT).cuda().eval()
        if extractor == 'SmoothGradCAMpp':
            self._cam_extractor = SmoothGradCAMpp(self._cam_model, layer)
        elif extractor == 'CAM':
            self._cam_extractor = CAM(self._cam_model, layer)
